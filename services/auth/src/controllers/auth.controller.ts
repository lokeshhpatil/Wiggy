...
const options: CookieOptions = {
  ...
  secure: isProd,
  maxAge: 15 * 60 * 1000,
  // Browsers require `Secure` when `SameSite='none'` — use 'lax' in dev.
...
export const userLogin = asyncHandler(async (req, res) => {
  ...
  if (!user.password) {
    throw new ApiError(500, 'Password not found.');
  }
  ...
  return res
    ...
    .json(new ApiResponse(200, { user: safeUser, token: accessToken }, 'login successful.'));
});
...
export const googleLogin = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .cookie('accessToken', accessToken, options)
    .json(new ApiResponse(200, { user: safeUser, token: accessToken }, 'Google login successful'));
});
...
export const updateUserRole = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(new ApiResponse(200, { updatedRole, token }, 'Role updated successfully'));
});

export const getProfile = asyncHandler(async (req, res) => {
  const user = req.user;
  res.status(200).json(new ApiResponse(200, user, 'MyProfile fetched successfully.'));
});
